...
export default async function inspect(client: Client, argv: string[]) {
    ...
    const [nameOrId] = argv;
    const target = link.project.customEnvironments?.find(t => t.slug === nameOrId || t.id === nameOrId);
    const projectSlugLink = formatProject(link.org.slug, link.project.name, '/settings/environments');
    if (!target) {
    ...
    output.print(`    ${chalk.cyan('Description')}\t\t${target.description || chalk.dim('(none)')}\n`);
    output.print(
    ...
    output.print(`    ${chalk.cyan('Status')}\t\t${target.branchMatcher ? 'Enabled' : 'Disabled'}\n`);
